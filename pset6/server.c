        ...
        if (connected())
        {
            ...
            if (request(&message, &length))
            {
                ...
                if (parse(line, abs_path, query))
                {
                    ...
                    {
                        printf("debug: abs_path = %s000\n", abs_path);
                        printf("debug: path = %s000\n", path);
                        printf("debug: query = %s000\n", query);
                        error(404);
                        continue;
                    }
...
{
    // TODO
    // strings for indexes, allocate memory and copy paths
    char* indexphp = malloc(strlen(path) + 1);
    strcpy(indexphp, path);
    char* indexhtml = malloc(strlen(path) + 1);
    strcpy(indexhtml, path);
    
    // resize string to include index.php or index.html extension
    indexphp = realloc(indexphp, strlen(indexphp) + 10);
    indexhtml = realloc(indexhtml, strlen(indexhtml) + 11);
    
    // concatenate path and index
...
{
    // TODO
    // int for length of file, instead of pointer
    int array = 0;
    // derefernce content and allocate memory
    *content = malloc(1000);
    // read from file into *content
    while (true)
    {
        // on each iteration read 1000 bytes, start reading where it left off
        int read = (fread(*content + array, 1, 1000, file));
        // keep track of length
        array = array + 1000;
        // if end of file is not reached, dynamically resize to fit next read
        if (read == 1000)
        {
            *content = realloc(*content, array + 1000);
        }
        // if end of file is reached, subtract unused space from array
        else if (read >= 0)
        {
            array = array - (1000 - read);
            break;
        }
    }
    // (sort of) return length of array
    *length = array;
    return true;
}
...
const char* lookup(const char* path)
{
    ...
    // return proper MIME type for extension
...
{
    // TODO
    // splits request line into three parts
    char* request = strchr(line, ' ');
    char* http = strstr(request, "HTTP");
    char* crlf = strstr(http, "\r\n");
    
    // if request line is not right format, or if request-target contains a "
    if (line[3] != ' ' || strchr(request, '\"') != NULL || http == NULL || strncmp(http - 1, " ", 1) != 0 || crlf == NULL)
    {
        error(400);
        return false;
    }
    ...
    // if request-target does not start with /
    if (request[1] != '/')
    {
        error(501);
        return false;
    ...
    // store absolute-path and query in separate strings
    char* querystart = strchr(request, '?');
    int querylength, pathlength;
    
    // if request-target contains "?"
    if (querystart != NULL)
    {
        // subtract 2 for space before http, and "?", which is not part of query
        querylength = http - querystart - 2;
        // copy query into string starting from character after "?"
        strncpy(query, querystart + 1, querylength);
        // add null 0 at end of query
        query[querylength] = '\0';
        // subtract 1 for space before request
        pathlength = querystart - request - 1;
        // copy abdolute-path into string, starting after space in request
        strncpy(abs_path, request + 1, pathlength);
        // add null 0 at end of abs_path
        abs_path[pathlength] = '\0';
    }
    // if request-target does not contain "?"
    else
    {
        // query = ""
        query = "\0";
        // subtract 2 for space before http, and space before request
        pathlength = http - request - 2;
        // copy abdolute-path into string, starting after space in request
        strncpy(abs_path, request + 1, pathlength);
        // add null 0 at end of abs_path
        abs_path[pathlength] = '\0';
